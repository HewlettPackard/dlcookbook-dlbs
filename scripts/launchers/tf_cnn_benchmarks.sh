...
if [ "$exp_status" = "simulate" ]; then
    echo "${tensorflow_env} ${runtime_launcher} python ${tensorflow_bench_path}/tf_cnn_benchmarks.py ${tf_cnn_args}"
    exit 0
fi
...
script="\
    ...
    ${runtime_launcher} ${runtime_python} ${tensorflow_bench_path}/tf_cnn_benchmarks.py ${tf_cnn_args} &\
    proc_pid=\$!;\
