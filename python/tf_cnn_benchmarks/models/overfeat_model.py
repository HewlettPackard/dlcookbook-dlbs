...
class OverfeatModel(model.Model):
  ...
  def add_inference(self, cnn):
    ...
    cnn.conv(96, 11, 11, 4, 4, mode='VALID')       # Layer1 (+relu)
    cnn.mpool(2, 2)
    cnn.conv(256, 5, 5, 1, 1, mode='VALID')        # Layer2 (+relu)
    cnn.mpool(2, 2)
    cnn.conv(512, 3, 3)                            # Layer3 (+relu)
    cnn.conv(1024, 3, 3)                           # Layer4 (+relu)
    cnn.conv(1024, 3, 3)                           # Layer5 (+relu)
    cnn.mpool(2, 2)
    cnn.reshape([-1, 1024 * 6 * 6])
    cnn.affine(3072)                               # Layer6 (+relu)
    cnn.dropout()
