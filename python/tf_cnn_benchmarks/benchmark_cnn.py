...
class BenchmarkCNN(object):
  ...
  def _benchmark_cnn(self):
    ...
    with sv.managed_session(
      ...
      log_fn('-' * 64)
      #
      print("__results.throughput__=%s" % (json.dumps(images_per_sec)))
      if num_steps > 0:
          dlbs_batch_time = 1000 * (float(elapsed_time) / num_steps)
          print("__results.time__=%s" % (json.dumps(dlbs_batch_time)))
      dlbs_milliseconds = [1000.0 * tm for tm in step_train_times]
      print("__results.time_data__=%s" % (json.dumps(dlbs_milliseconds)))
      #
      image_producer.done()
      if is_chief:
