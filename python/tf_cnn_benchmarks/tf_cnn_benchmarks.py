...
class ConvNetBuilder(object):
  ...
  def lrn(self,
          ...
          input_layer=None,
          num_channels_in=None):
    """Construct Local Resonse Normalization layer"""
    if input_layer is None:
      input_layer = self.top_layer
    else:
    ...
    self.top_layer = tf.nn.local_response_normalization(
      input_layer,
      depth_radius=radius,
      alpha=alpha,
      beta=beta,
  ...
  def reshape(self, shape, input_layer=None):
    ...
    log_fn('Reshaped input to ' + str(self.top_layer.shape) + ' with self.top_size = ' + str(self.top_size))
    return self.top_layer
...
def loss_function(logits, labels):
  ...
  log_fn('Adding sparse softmax cross entropy with logits with logits shape ' + str(logits.shape) + ' and labels shape ' + str(labels.shape))
...
def add_image_preprocessing(dataset, input_nchan, image_size, batch_size,
                            ...
                            resize_method, train, model=None):
  ...
  if dataset is not None:
    ...
    # Why do they used 1001 and not 1000? Read this (https://github.com/tensorflow/models/tree/master/slim)
    # Basically, in ImageNet dataset we have 1001 classes where that additional class is a background class.
    if model is not None:
	if model == 'deep_mnist':
	    nclass = 10     # Digit classes
	elif model == 'sensor_net':
	    nclass = 16     # Number of activities
	elif model == 'eng_acoustic_model':
    	    nclass = 8192   # Clustered tri-phones classes
	else:
    	    nclass = 1001   # Image classes + one backgraound class
    else:
	nclass = 1001
...
class BenchmarkCNN(object):
  ...
  def __init__(self):
    ...
    print("BenchmarkCNN::__init__ time=%f ms" % (1000.0*(timeit.default_timer() - start_time)))
  ...
  def _build_model(self):
    ...
    if self.model == 'deep_mnist' or self.model == 'eng_acoustic_model' or self.model == 'sensor_net':
        input_nchan = 1
    else:
        input_nchan = 3
    ...
    log_fn('Adding preprocessing for ' + self.model)
    with tf.device(self.cpu_device):
      nclass, images_splits, labels_splits = add_image_preprocessing(
          ...
          not FLAGS.eval, model=self.model)
  ...
  def add_forward_pass_and_gradients(
    ...
    with tf.device(self.devices[device_num]):
      ...
      log_fn('Added final fully connected layer with logits shape ' + str(logits.shape))
      if not phase_train:
