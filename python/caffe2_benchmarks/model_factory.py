...
"""Model factory that creates Caffe2 model builders.
...
def import_models():
    ...
    for fname in fnames:
        ...
        if mname.startswith('__'):
            continue
        module = importlib.import_module("caffe2_benchmarks.models." + mname)
...
class ModelFactory(object):
    """Caffe2 model factory that creates models (actually, creates classes that knows how to create models)."""
    
    models = import_models()

    @staticmethod
    def get_model(params):
        """Return model builder identified by *params['model']*.
        
        :param dict params: Prameters of the model. Must include at least *model* 
                            key that identifies model id.
        :return: Model instance. It's actually a 'model builder' that creates model
                 with method 'forward_pass_builder'.
        """
